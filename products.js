window.PRODUCTS = [
  {
    group: "VacPress",
    description: "Cup-type piston cold brew coffee makers. The piston creates positive and negative pressure to enhance extraction in cold water and makes cold brew coffee in 3–5 minutes.",
    shipping: "USA shipping: Standard $8, Express $25 for one or more VacPress products.",
    items: [
      {
        name: "VacPress Glass Cup, 200 ml",
        price: "$30.00",
        specs: ["Borosilicate glass", "Single-wall clear glass cup", "200 ml"],
        images: ["Piston press glass.jpg"],
        videos: ["VacPressDemo.mp4"],
        stripe: "https://buy.stripe.com/test_28E28k65E8FO2hz0CB24000",
        paypal: "https://www.paypal.com/ncp/payment/6E5NU2GE5AUH6"
      },
      {
        name: "VacPress Stainless Thermos Cup, 300 ml",
        price: "$35.00",
        specs: ["304 stainless steel", "Double-wall thermos cup", "300 ml"],
        images: ["Piston press SS photo.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_fZu5kwfGeg8g7BTgBz2400v",
        paypal: "https://www.paypal.com/ncp/payment/5U5ZG8DTW75XA"
      }
    ]
  },
  {
    group: "VacBrew",
    description: "Portable cold coffee extractors using steam-condensed vacuum. The vacuum pulls icy cold water through coffee grind to make fresh, flavorful, 2–5 times concentrated coffee.",
    shipping: "USA shipping: Standard $19.99, Express $44.99 for one or more VacBrew products.",
    items: [
      {
        name: "VacBrew Personal Glass, 500 ml",
        price: "$65.00",
        specs: ["Borosilicate glass", "Up to 500 ml", "2–3 times concentrated coffee"],
        images: ["VacBrewGS.PNG"],
        videos: ["VacBrewDemo.mp4"],
        stripe: "https://buy.stripe.com/test_bJefZabpYcW45tL3ON24002",
        paypal: "https://www.paypal.com/ncp/payment/7SUWQSVUGSZZJ"
      },
      {
        name: "VacBrew Glass Large, 2,000 ml",
        price: "$180.00",
        specs: ["Borosilicate glass", "Up to 2000 ml", "For office or coffee shop display"],
        images: ["Glass Large.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_5kQ28k8dM1dm1dv1GF2400u",
        paypal: "https://www.paypal.com/ncp/payment/S3392W5A8FTGY"
      },
      {
        name: "VacBrew Stainless Small Cartridge",
        price: "$130.00",
        specs: ["304 stainless steel", "Up to 1500 ml", "2–5 times concentrated coffee"],
        images: ["SSF.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_4gMeV6gKi2hq2hzgBz2400t",
        paypal: "https://www.paypal.com/ncp/payment/AXGSHEF9YHMDE"
      },
      {
        name: "VacBrew Stainless Large Cartridge",
        price: "$170.00",
        specs: ["304 stainless steel", "Up to 3500 ml", "3–5 times concentrated coffee"],
        images: ["Stainless Large.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_8x2cMYbpY1dm2hzdpn2400s",
        paypal: "https://www.paypal.com/ncp/payment/TFALACEH3FFJE"
      }
    ]
  },
  {
    group: "VacBrew Commercial",
    description: "Commercial vacuum pump extractors for icy cold water coffee extraction. Vacuum extraction helps reduce oxidation and preserve coffee flavor and aroma.",
    shipping: "Standard shipping: $200 / $250 / $450 / $750 / $1200 depending on model. Optional 7-day delivery, setup, and training available.",
    items: [
      {
        name: "VacBrew Commercial 7-Gal Light Weight",
        price: "$2,300.00",
        specs: ["304 stainless steel", "7-Gal", "28 kg", "Low vacuum, up to 10 times concentration"],
        images: ["VacBrew 7 Gal.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_dRm28k8dMe08g8p5WV24003",
        paypal: "https://www.paypal.com/ncp/payment/DCGDQKLZ2HLQ8"
      },
      {
        name: "VacBrew Commercial 15-Gal Light Weight",
        price: "$2,800.00",
        specs: ["304 stainless steel", "15-Gal", "33 kg", "Low vacuum, up to 10 times concentration"],
        images: ["VacBrew 15 gals.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_7sY6oA9hQ9JSaO5gBz2400r",
        paypal: "https://www.paypal.com/ncp/payment/ZNNW6FR3BUWHS"
      },
      {
        name: "VacBrew Commercial 7-Gal Heavy Duty",
        price: "$3,500.00",
        specs: ["304 stainless steel", "7-Gal", "85 kg", "High vacuum, up to 15 times concentration"],
        images: ["VacBrew 7 Gal Hard Extracter.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_28E28k8dM9JScWdgBz2400o",
        paypal: "https://www.paypal.com/ncp/payment/LSFXHEB2NASSN"
      },
      {
        name: "VacBrew Commercial 20-Gal Heavy Duty",
        price: "$5,800.00",
        specs: ["304 stainless steel", "20-Gal", "115 kg", "High vacuum, up to 15 times concentration"],
        images: ["CCommercial 20Gal.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_00wfZa0LkcW43lDetr2400p",
        paypal: "https://www.paypal.com/ncp/payment/FSX2NQFN4J8VJ"
      },
      {
        name: "VacBrew Commercial 50-Gal Heavy Duty",
        price: "$11,500.00",
        specs: ["304 stainless steel", "50-Gal", "225 kg", "High vacuum, up to 15 times concentration"],
        images: ["VacBrewCommercial50Gal.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_6oU8wIgKiaNWe0h5WV2400q",
        paypal: "https://www.paypal.com/ncp/payment/WQHWCVA7XDXAE"
      }
    ]
  },
  {
    group: "Concentrated Cold Brew Coffee",
    description: "5 times and 10 times concentrated cold brew coffee extracted with icy cold water by VacBrew Commercial Extractor.",
    shipping: "USA shipping: Standard $12.99, Express $35 for one or more 32 oz glass bottle products.",
    items: [
      {
        name: "32 oz 5x Concentrated Cold Brew Coffee",
        price: "$13.00",
        specs: ["Amber glass bottle", "5 times concentrated"],
        images: ["CCBottle.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_5kQ7sE2Ts9JS09r3ON2400e",
        paypal: "https://www.paypal.com/ncp/payment/J3ARNBL4HHJYG"
      },
      {
        name: "32 oz 10x Concentrated Cold Brew Coffee",
        price: "$23.00",
        specs: ["Amber glass bottle", "10 times concentrated"],
        images: ["CCBottle.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_cNi7sE79I6xGbS9dpn2400d",
        paypal: "https://www.paypal.com/ncp/payment/M59AD4ATRX6CQ"
      }
    ]
  },
  {
    group: "SafelyRoasted",
    description: "Organically grown Ethiopia origin coffee beans roasted with more time and effort, in a lower oxygen environment, to preserve flavor and aroma.",
    shipping: "Standard shipping: $7.99 half lb, $9.99 1 lb, $17.99 3 lbs, $23 5 lbs, $38 10 lbs. Express shipping available.",
    items: [
      {
        name: "SafelyRoasted Coffee Bean 1/2 lb",
        price: "$12.50",
        specs: ["Organically grown Ethiopia origin coffee bean"],
        images: ["Logo coffee.jpg", "SafelyRoastedHalf.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_14A3cofGe5tCf4l4SR2400j",
        paypal: "https://www.paypal.com/ncp/payment/KUUPUHDQHZ2L8"
      },
      {
        name: "SafelyRoasted Coffee Bean 1 lb",
        price: "$23.00",
        specs: ["Organically grown Ethiopia origin coffee bean"],
        images: ["Logo coffee.jpg", "SafelyRoasted1lb.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_bJefZa65E6xG1dv8532400i",
        paypal: "https://www.paypal.com/ncp/payment/3347LFE9VX5BJ"
      },
      {
        name: "SafelyRoasted Coffee Bean 3 lbs",
        price: "$65.00",
        specs: ["Organically grown Ethiopia origin coffee bean"],
        images: ["Logo coffee.jpg", "SafelyRoasted3lbs.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_3cI14g51AcW46xPfxv2400h",
        paypal: "https://www.paypal.com/ncp/payment/SKMTFAGTD53GG"
      },
      {
        name: "SafelyRoasted Coffee Bean 5 lbs",
        price: "$105.00",
        specs: ["Organically grown Ethiopia origin coffee bean"],
        images: ["Logo coffee.jpg", "SafelyRoasted5lbs.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_fZu8wI1Po09if4ldpn2400g",
        paypal: "https://www.paypal.com/ncp/payment/KGGJU4RDH4VYJ"
      },
      {
        name: "SafelyRoasted Coffee Bean 10 lbs",
        price: "$200.00",
        specs: ["Organically grown Ethiopia origin coffee bean"],
        images: ["Logo coffee.jpg", "SafelyRoasted10lbs.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_28EeV6gKig8g3lD2KJ2400f",
        paypal: "https://www.paypal.com/ncp/payment/ZRRTD6UHGKEPY"
      }
    ]
  },
  {
    group: "E-Z Braider",
    description: "A professional hair braiding machine that braids hair at 750 RPM and can braid many sizes from micro to jumbo braid.",
    shipping: "USA shipping: Standard $15, Express $35 for one or more E-Z Braider products.",
    items: [
      {
        name: "E-Z Braider Basic Set",
        price: "$150.00",
        specs: ["750 RPM", "Braids 3 times faster than a professional hair braider", "For micro to jumbo braid"],
        images: ["ezbasic.PNG", "EZ Braider long hair.PNG", "ezbtreebraid.PNG", "ezfuhair.PNG", "ezbraidercolorext.PNG", "ezjump.PNG"],
        videos: ["eznatural.mp4", "ezextension.mp4", "ezcoloryam.mp4", "ezfuson.mp4"],
        stripe: "https://buy.stripe.com/test_aFaeV6gKi1dm6xPclj24006",
        paypal: "https://www.paypal.com/ncp/payment/AHTVPLJBPTR3J"
      }
    ]
  },
  {
    group: "100% Whole Grain Brown Rice Snack",
    description: "Whole grain brown rice snacks. Not fried in oil. Plain has no added sugar. Brown rice has low GI and contains antioxidants, vitamins, and minerals.",
    shipping: "Standard shipping: $15 up to 5 bags, $25 up to 10 bags, $35 up to 20 bags, $50 one case. Express and 7–10 day delivery options available.",
    items: [
      {
        name: "Brown Rice Snack Plain Small",
        price: "$3.00",
        specs: ["100% whole grain brown rice", "Plain original", "Small size"],
        images: ["100%BRPlainS.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_bJe00cfGe09i2hz8532400k",
        paypal: "https://www.paypal.com/ncp/payment/ABND5CRPG96CE"
      },
      {
        name: "Brown Rice Snack Plain Medium",
        price: "$5.00",
        specs: ["100% whole grain brown rice", "Plain original", "Medium size"],
        images: ["100%BRPlainM.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_fZu8wI0Lk5tC7BTbhf2400l",
        paypal: "https://www.paypal.com/ncp/payment/EWRCKDNNMDNHN"
      },
      {
        name: "Brown Rice Snack Cinnamon Small",
        price: "$4.00",
        specs: ["Whole grain brown rice", "Cinnamon with lightly added brown sugar", "Small size"],
        images: ["BRCinnamon.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_bJe00cfGe09i2hz8532400k",
        paypal: "https://www.paypal.com/ncp/payment/6W48YBYAAD45U"
      },
      {
        name: "Brown Rice Snack Coffee Small",
        price: "$4.00",
        specs: ["Whole grain brown rice", "Coffee flavor with lightly added brown sugar", "Small size"],
        images: ["BRCoffee.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_6oU14gfGecW48FX1GF2400m",
        paypal: "https://www.paypal.com/ncp/payment/P92GX3EMD4B4U"
      }
    ]
  },
  {
    group: "I Brew",
    description: "Future product: permanently reusable cold brew cartridge. Fresh cold coffee can be brewed by your own mouth suction power, with no disposable plastic or paper cups.",
    shipping: "Future product. Production may begin with large quantity orders.",
    items: [
      {
        name: "I Brew Cartridge for PET Water Bottle",
        price: "$13.00",
        specs: ["Reusable cold brew cartridge", "For PET water bottle", "Future product"],
        images: ["IBrewwaterbottle.PNG"],
        videos: [],
        stripe: "https://buy.stripe.com/test_14AfZafGe8FO5tL70Z2400a",
        paypal: "https://www.paypal.com/ncp/payment/ZA4WL73RT5SEW"
      },
      {
        name: "I Brew Cartridge with 500 ml Glass Bottle",
        price: "$30.00",
        specs: ["Reusable cold brew cartridge", "Reusable 500 ml glass bottle", "Future product"],
        images: ["I Brew Glass.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_00w7sEcu24py8FX99724007",
        paypal: "https://www.paypal.com/ncp/payment/R3PR8DSZYP668"
      },
      {
        name: "I Brew Cartridge with 500 ml Stainless Bottle",
        price: "$35.00",
        specs: ["Reusable cold brew cartridge", "500 ml double-wall stainless bottle", "Future product"],
        images: ["I Brew SS.jpg"],
        videos: [],
        stripe: "https://buy.stripe.com/test_cNieV679I6xG8FXadb2400c",
        paypal: "https://www.paypal.com/ncp/payment/36TM72B5WPJQ8"
      }
    ]
  },
  {
    group: "Blade Tilting Wind Turbine",
    description: "Future product: high torque at low wind speed and nonstop operation at high wind speed. Designed for quiet, safe, low-RPM operation.",
    shipping: "FOB Korea. Production may begin when there are more than 10 orders for each size.",
    items: [
      {
        name: "Blade Tilting Type Wind Turbine 5 KW",
        price: "$23,000.00",
        specs: ["Up to 40% efficiency", "Starts generation at 3 m/second wind speed", "Quiet operation", "Low RPM, high torque", "Future product"],
        images: ["WTBT5KW.PNG"],
        videos: ["WindTerbine1.mp4", "WindTerbine2.mp4"],
        stripe: "https://buy.stripe.com/test_3cIcMY3Xw3lu9K12KJ24009",
        paypal: "https://www.paypal.com/ncp/payment/5R6N55ERTC6RQ"
      }
    ]
  }
];
